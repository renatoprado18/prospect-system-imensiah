...
def test_task_na_ficha_do_dono_nao_traz_o_email_dele_pro_escopo(
    cursor_caso_piccino, monkeypatch
):
    """REGRESSAO ACHADA NO GATE DE MERGE (22/08) — a maquina ouvindo o proprio eco.

    A subtracao de `owner_emails` existia so no degrau do TEXTO (task sem ficha).
    O degrau da FICHA nao subtraia nada, e isso importa porque **21 das 148 tasks
    pending apontam pra ficha #23419 — a do proprio Renato** (as 4 fichas dele
    foram fundidas numa em 06/08). Para essas, o escopo saia com os enderecos
    DELE, e a perna de e-mail passava a casar qualquer mensagem que ele mandou ou
    recebeu: 266 e-mails entrando como "o terceiro respondeu".
    ...
    scope_ok = tr._task_scope(
        {"id": 999695, "titulo": "FUP Piccino", "descricao": "", "contact_id": 2869}
    )
    assert "joao@piccino.com.br" in scope_ok["emails"]
