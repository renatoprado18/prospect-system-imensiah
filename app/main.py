...
async def api_criar_ordem(veiculo_id: int, request: Request):
    ...
    # Nova OS pendente cobre itens — invalida cache pra refletir no statcard
    global _veiculos_alertas_cache
    _veiculos_alertas_cache = {"data": None, "timestamp": None}
    return os
...
async def api_finalizar_ordem(os_id: int, request: Request):
    ...
    # Mudanca de status da OS afeta calculo de "vencidos_em_execucao" no statcard
    global _veiculos_alertas_cache
    _veiculos_alertas_cache = {"data": None, "timestamp": None}
    return resultado


@app.post("/api/veiculos/ordens-servico/{os_id}/iniciar")
async def api_iniciar_ordem(os_id: int):
    """
    Transiciona OS de 'pendente' pra 'em_andamento' (usuario levou pra oficina).
    Mantem o item no statcard como "em OS" pra parar de gritar urgente.
    """
    from services.veiculos import iniciar_ordem_servico
    resultado = iniciar_ordem_servico(os_id)
    if resultado.get('error'):
        raise HTTPException(status_code=400, detail=resultado['error'])
    # Invalida cache de alertas pra refletir mudanca de status no dashboard
    global _veiculos_alertas_cache
    _veiculos_alertas_cache = {"data": None, "timestamp": None}
...
async def api_deletar_ordem(os_id: int):
    ...
    if resultado.get('error'):
        raise HTTPException(status_code=400, detail=resultado['error'])
    # Itens deixam de estar cobertos por OS — invalida cache do statcard
    global _veiculos_alertas_cache
    _veiculos_alertas_cache = {"data": None, "timestamp": None}
    return resultado
...
@track_cron_run
async def cron_cos_portfolio_tick(request: Request):
    """Cron CoS Portfolio Agent (15/06/26). Specialist #2 do swarm.

    Drift detector + revisao estrategica horizontal de TODOS os projetos INTEL
    ativos. NAO duplica analise dos 4 conselhos (CONSELHEIRO cobre).
    Schedule via Railway: 1x/dia 13 UTC = 10h BRT. Budget cap: $0.50/dia.
    """
    if not verify_cron_auth(request):
        raise HTTPException(status_code=401, detail="Unauthorized cron request")
    from services.cos_portfolio import tick_safe
    try:
        result = tick_safe()
        return {"job": "cos-portfolio-tick", **result}
    except Exception as e:
        logging.exception("cos-portfolio-tick falhou")
        return {"job": "cos-portfolio-tick", "status": "error", "error": str(e)}


@app.post("/api/cos/portfolio/run")
async def manual_cos_portfolio_run(request: Request):
    """Trigger manual do PORTFOLIO. Aceita user session OU WORKER_SECRET."""
    worker_secret_env = (os.getenv("WORKER_SECRET", "intel-audio-2026") or "").strip()
    triggered_by = None
    try:
        body = await request.json()
    ...
    try:
        result = tick_safe()
        return {"job": "cos-portfolio-manual", "triggered_by": triggered_by, **result}
    except Exception as e:
        logging.exception("manual cos-portfolio falhou")
        return {"job": "cos-portfolio-manual", "status": "error", "error": str(e)}
