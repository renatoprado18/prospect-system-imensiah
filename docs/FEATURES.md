...
- **Atuadores novos** (`app/services/actuators.py`, endpoint `POST /api/actuators/execute`, whitelist `ALLOWED_ACTIONS`): além de `create_task`/`schedule_wa`, agora `send_push` (Web Push/VAPID broadcast via `push_notifications.get_push_service`), `send_email` (Gmail via `google_accounts` personal/professional + `get_valid_token`), `create_calendar_event` (`calendar_events.CalendarEventsService`, resolve conta/token internamente). Espelhados como tools no toolset da Tônia (`tonia/app/services/actuator_tools.py`) — a Tônia AGE nesses canais sob comando.
