...
- **Service `app/services/capability_registry.py`** — `build_registry(days=14) -> dict` retorna 4 seções, cada capacidade com `{capability_key, capability_type, cost_usd, invocations, value_acted, value_ignored, value_ratio, extra}`. Os 4 tipos e como cada um mede VALOR:
  ...
  - **llm_function** (via `tonia_llm_usage` por `endpoint` + `platform_costs`): custo LLM por função (ex `briefing.generate`, `urgent.judge`). **Sem proxy de valor por-chamada → `value_ratio=None`** (explicado no extra). Adiciona 1 capacidade agregada `llm:anthropic-platform` do `platform_costs` (custo Anthropic que ainda NÃO é atribuído por-função — hoje só 2 endpoints estão instrumentados).
