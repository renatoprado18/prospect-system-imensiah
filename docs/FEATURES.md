...
- **Follow-up (documentado, não feito):** approve por linguagem natural no bot reativo ("aprovo playbook", espelhando o loop RACI de `9237cb3`); surfaçar propostas pendentes na abertura `/dev`; usar transcrição além do resumo na extração; rótulo de custo LLM próprio (`playbook.*`) em vez de cair no `triage.advisor` do `_call_model`.
