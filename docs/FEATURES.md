...
- **Sinal `cruzamento_noticia_contato`** (v0): notícia recente (janela 14d, `project_news_hits`) sobre uma entidade vigiada (`empresa_relacionada` ou `watcher_query` — nunca o nome do projeto, que carrega nome de pessoa) onde o Renato TEM contatos no CRM (`contacts.empresa`, match por início-de-palavra com `unaccent`, token ≥4 chars, stopword-filtered). **Agregado por projeto** (1 signal/entidade, não por manchete), contexto = entidade + manchetes (≤5) + contatos (≤8, ordenados por círculo) + ângulo de reaproximação. **Só emite com ≥1 contato casado.** Urgência 5-6 → **briefing** (nunca urgent; anti-ruído gen-1). Dedup por `signal_hash(project_id)`, `expire_stale_signals` quando a entidade sai da janela. A Tônia consome via `copilot.signals` no briefing (mecanismo F-A). Sem migration (lê tabelas existentes).
