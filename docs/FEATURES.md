...
- **Instrumentação de custo por-função — `app/services/llm_usage.py`** (20/07/26, fecha o "$32 cego" da retro F-E): o lado-INTEL do custo LLM ficava cego porque a maioria dos ~35 call-sites (`httpx.post` direto pra `api.anthropic.com/v1/messages`, sem wrapper central) NÃO gravava em `tonia_llm_usage` — só briefing/urgent da Tonia (Railway) gravavam. Este módulo é o ponto único de gravação pro lado-INTEL: `record_response(function, model, response_json)` extrai o `usage` da resposta, computa custo (tabela de preços haiku/sonnet/opus == skill claude-api) e faz INSERT em `tonia_llm_usage` (`endpoint`=função). **Aditivo e defensivo** — try/except silencioso, nunca quebra a chamada de LLM. 31 call-sites instrumentados em 19 services (OCR `ocr.screenshot`/`photo.validate`, `smart_update.*`, `raci.*`, `enrichment.*`, `intelligence.contact`, `briefing.*`, digests, `hot_takes.*`, editorial, `wa_triage.classify`, `news_watcher.*`, `meeting.suggest`). Débito conhecido: ~13 call-sites ainda cegos (10 em `main.py`, 3 via SDK — `cos_investigator`/`linkedin_comment_curator`/`linkedin_enrichment`).
